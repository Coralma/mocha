...
public class InsuranceBrokerControllerMenuPanel extends ControllerMenuPanel {
	...
	public void build() {
		try {
			...
			menuItem = createMenu("Followed Members", action);
			addComponent(menuItem);
			action = new MenuAction();
			action.setPanel(Class.forName("com.mocha.businessSchool.BusinessSchoolHomePresenter"));
			menuItem = createMenu("Business Tools", action);
			addComponent(menuItem);
			action = new MenuAction();
			action = new MenuAction();
			action.setPanel(Class.forName("com.mocha.soicalAPI.PeopleSearchPrensenter"));
