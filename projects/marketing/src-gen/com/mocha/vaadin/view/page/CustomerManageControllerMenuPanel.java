...
public class CustomerManageControllerMenuPanel extends ControllerMenuPanel {
	...
	public void build() {
		try {
			...
			addComponent(menuItem);
			groupTitle = createMenuTitle("Email Marketing Center");
			addComponent(groupTitle);
			action = new MenuAction();
			action.setPanel(Class.forName("com.mocha.emailMarketing.CreateEmailPresenter"));
			menuItem = createMenu("Create Campaign", action);
			addComponent(menuItem);
			action = new MenuAction();
			action.setPanel(Class.forName("com.mocha.emailMarketing.ViewCampaignsPrenseter"));
			menuItem = createMenu("View Campaigns", action);
			addComponent(menuItem);
