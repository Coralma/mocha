...
public class SimplePublisherWidget extends VerticalLayout implements ClickListener {

	private static final long serialVersionUID = 3182399657534581927L;
	private String widgetWidth = "768px";
	private String inputWidgetWidth = "766px";
	private VerticalLayout layout = new VerticalLayout();
	private AttachmentLayout attachmentLayout;
//	private NativeButton timesheetBtn;
	...
	public SimplePublisherWidget(BasicUser currentUser) {
		this.currentUser = currentUser;
		this.setWidth("770px");
//		this.addStyleName("publisher-widget");
		layout.addStyleName("s-publisher");
		layout.setSizeFull();
	}
...
//		timesheetBtn= createButton("Timesheet","icons/timesheet_icon.png");
		updateStatusBtn = createButton("My Status","icons/status_icon.png");
...
//		todoBtn = createButton(message.getString("cooperate.publisher.newTask"),"icons/todo-icon.png");
		discussBtn = createButton("Let's Discuss","icons/discuss_icon.png");
		todoBtn = createButton("New Task","icons/todo-icon.png");
//		changeSelectedStyle(timesheetBtn);
...
//		createTypeLayout.addComponent(timesheetBtn);
...
//		currentWidget = buildTimesheetInput();
...
//		upload.setIcon(new ThemeResource("icons/file_icon.png"));
	...
	public IPublisher buildTimesheetInput() {
		IPublisher inputArea = new TimesheetPublisher();
		inputArea.build();
		return inputArea;
	}
...
//			if (event.getButton().equals(timesheetBtn)) {
//				selectedComponent = buildTimesheetInput();
//			} else 
	...
	public class TimesheetPublisher extends VerticalLayout implements IPublisher {
		private static final long serialVersionUID = 6368116528860778991L;
		private ExpandingTextArea inputArea = new ExpandingTextArea();
		
		public TimesheetPublisher() {
			this.setWidth(widgetWidth);
			this.addStyleName("publisher-layout");
		...
		public void build() {
			...
			String[] types = new String[]{"Workday","Leave"};
			CodeTableField type = new CodeTableField("Activity Type", types);
			type.setValue(types[0]);

			headLayout.addComponent(type);
			this.addComponent(headLayout);
			
			inputArea.setInputPrompt("Work Description");
			inputArea.setWidth(inputWidgetWidth);
			inputArea.setRows(2);
	...
	public class DiscussPublisher extends VerticalLayout implements IPublisher {
		...
		public void build() {
			titleField.setWidth(inputWidgetWidth);
