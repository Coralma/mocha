...
public class SocialPullAsyncThread extends Thread {
   ...
   * item.
   */
  public SocialPullAsyncThread(BlockingQueue<Runnable> workQueue) {
    this(workQueue, 1);
  }
   ...
   *        low.
   */
  public SocialPullAsyncThread(BlockingQueue<Runnable> workQueue, int threadLocalItems) {
    this.workQueue = workQueue;
    this.threadLocalItems = threadLocalItems;
    internalQueue = new ArrayDeque<Runnable>();
    setDaemon(true);
    synchronized (SocialPullAsyncThread.class) {
