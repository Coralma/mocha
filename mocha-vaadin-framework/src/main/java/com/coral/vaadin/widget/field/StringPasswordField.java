 ...
 *
 */
public class StringPasswordField extends AbstractField implements Field {

	private static final long serialVersionUID = -8142790346061175420L;
//	private TextField stringField = new TextField();
	
	public StringPasswordField(String label) {
		super(label);
		field = new PasswordField();
		((PasswordField)field).setImmediate(true);
...
//	}
	...
	public void setError(boolean isError, String errorMsg) {
		if(isError) {
			((PasswordField)field).setComponentError(new UserError(errorMsg));
		} else {
			((PasswordField)field).setComponentError(null);
		}
	}
}
