...
public class DateUtils {
	...
	private static int DAYS_OF_MONTH = 31;
	 ...
	 */
	public static int getShortYear(java.util.Date date) {
		int shotYear = 0;
		String year = getYear(date) + "";
		int length = year.length();
		year = year.substring(length - 2, length);
		if (year != null) {
			shotYear = Integer.parseInt(year);
		}
		return shotYear;
	}
	...
	public static Date roundMonth(Date date) {
		return org.apache.commons.lang.time.DateUtils.round(date,
				Calendar.MONTH);
	}
	...
	public static String currentDateSQLString() {
	...
	public static Calendar createCalendar() {
		...
		/*
		 * AppUser user = null; try { user = AppUserContext.getCurrentUser(); }
		 * catch (RTException ex) { if (ThreadBindResourceManager
		 * .hasBindedResource(WebConstant.NO_APPUSER_TRUE)) { } throw ex; }
		 * String organId = user.getOrganId(); if (organId == null) { throw new
		 * RTException("Wrong organ id for user :" + user); } String timeZone =
		 * Para.getParaValueAlwaysToCache( ParaId.PUB_ORGNA_TIME_ZONE, organId,
		 * null, null); if (null == timeZone) { result = Calendar.getInstance();
		 * } else { TimeZone tz = TimeZone.getTimeZone(timeZone); // no use
		 * timezone now result = Calendar.getInstance(); }
		 */
		result = Calendar.getInstance();
		return result;
	}

	public static List<Integer> getAroundYear(Date startDate, Date endDate) {
		int startYear = getYear(startDate) - 1;
		int endYear = getYear(endDate);
		List<Integer> aroundYears = new ArrayList<Integer>();
	...
	public static List<Integer> getAroundMonth(Date date) {
		List<Integer> aroundMonths = new ArrayList<Integer>();
	...
	public static Date getCurrentYearMonthLastDate(int month) {
	...
	public static Date getYearLastDate(int year) {
		return toDate(year, 12, 31, 23, 59, 59);
	}

	public static void main(String[] args) {
//		System.out.println(date2String(new Date(),"yyyy-mm-dd hh:mm:ss"));
//		List<Integer> years = getAroundYear(new Date(), toDate(2000, 1, 1, 1, 1, 1));
//		List<Integer> years = getAroundYear(new Date(), new Date());
//		System.out.println(years);
		
//		List<Integer> months = getAroundMonth(new Date());
//		System.out.println(months);
		
		System.out.println(getCurrentYearMonthLastDate(4));
		System.out.println(getYearLastDate(2012));
