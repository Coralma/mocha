 ...
 * 
 * 1. Create an ApiContext Object 2. Set the auth token and target api url (Webservice endpoint) 3. Create a GetTransactionsCall object. 4. Set the ModifiedFrom
 * and ModifiedTo filter (just an example) 5. Set Pagination Parameters 6. Do an iteration till all transactions for the specified filter is retrieved (in this
 * example we'll be using do..while ) 7. Check if the transaction is part of an order. 8. If yes, add it to a list. 9. Once all transactions are retrieved,
 * check if the transaction list contains any entry 10. If Yes, execute the GetOrders call with all the orders. 11. Replace the transaction level information
...
public class ManageTransactions {
	...
	public String createRedirectURLByRuName(String ruName) {
		...
		try {
			String signInURL = signInBaseURL + "&runame=" + URLEncoder.encode(ruName, "ISO-8859-1") + "&sid=" + sid + "&SessID=" + getSessionIDByRuName(ruName);
			setSecretID(sid);
	...
	public List<OrderType> getSalesTransactionsByEbayToken(String ebayToken) throws ApiException, SdkException, Exception {
		...
		GetSellerTransactionsCall getTrans = new GetSellerTransactionsCall(apiContext);
		...
		do {
			...
			System.out.print(" *********************************");
			System.out.print("Page " + pageNumber + " of " + getTrans.getPaginationResult().getTotalNumberOfPages());
			System.out.println("*********************************");
			if (getTrans.getReturnedTransactionCountActual() > 0) {
				for (TransactionType transaction : transactions) {

					System.out.println(" Transaction " + ++tranCounter + " of " + getTrans.getPaginationResult().getTotalNumberOfEntries());

					System.out.println(" BuyerId: " + transaction.getBuyer().getUserID());

					System.out.println("Transaction ID " + transaction.getTransactionID() + " ItemID: " + transaction.getItem().getItemID() + " Item Title : "
							+ transaction.getItem().getTitle());

					System.out.println("Quantity: " + transaction.getQuantityPurchased());

					System.out.println("Transaction Price : " + transaction.getTransactionPrice().getValue() + " CheckOut Status : "
							+ transaction.getStatus().getCheckoutStatus().value() + " PaymentStatus : "
							+ transaction.getStatus().getEBayPaymentStatus().value());

					System.out.println("CreatedDate : " + dateFormatter.format(transaction.getCreatedDate().getTime()) + " Last Time Modified : "
							+ dateFormatter.format(transaction.getStatus().getLastTimeModified().getTime()));
					if (transaction.getShippingDetails().getSalesTax().getSalesTaxAmount() != null) {
						AmountType tax = transaction.getShippingDetails().getSalesTax().getSalesTaxAmount();
					...
					if (transaction.getFinalValueFee() != null) {
						...
						System.out.println("Final Value Fee : " + finalValueFee.getValue());
					}

					if (transaction.getStatus().getCheckoutStatus().equals(CheckoutStatusCodeType.CHECKOUT_COMPLETE)) {// To check if the item is ready for
																														// shipping
						System.out.format("Payment Method: %s Total: %s Adjustment Amount: %s ", transaction.getStatus().getPaymentMethodUsed().value(),
								transaction.getAmountPaid().getValue(), transaction.getAdjustmentAmount().getValue());
						if (transaction.getShippingServiceSelected() != null) {
							System.out.println("Shipping Service: " + transaction.getShippingServiceSelected().getShippingService());
							if (transaction.getShippingServiceSelected().getShippingServiceCost() != null) {
								System.out.println("Shipping Cost: " + transaction.getShippingServiceSelected().getShippingServiceCost().getValue());
							}
						}

						AddressType shippingAddress = transaction.getBuyer().getBuyerInfo().getShippingAddress();
						...
						if (shippingAddress.getCityName() != null) {
							address.append(",").append(shippingAddress.getCityName());
						}
						if (shippingAddress.getStateOrProvince() != null) {
							address.append(",").append(shippingAddress.getStateOrProvince());
						}
						if (shippingAddress.getPostalCode() != null) {
							address.append(",").append(shippingAddress.getPostalCode());
						}
						if (shippingAddress.getCountryName() != null) {
							address.append(",").append(shippingAddress.getCountryName());
						}
					...
					if (transaction.getPaidTime() != null) {
						System.out.println("Paid Time:" + dateFormatter.format(transaction.getPaidTime().getTime()));
					}

					if (transaction.getShippedTime() != null) {
						System.out.println("Shipped Time: " + dateFormatter.format(transaction.getShippedTime().getTime()));
					}
					...
					if (transaction.getContainingOrder() != null) {
						if (!orderIdList.contains(transaction.getContainingOrder().getOrderID())) {
							...
							orderIdList.add(orderID); // Collect all the orders
														...
														// one call with
														// GetOrders.
							System.out.println("This transaction is part of the order : " + orderID);
						}
					}
					System.out.println();
				}
		...
		if (!orderIdList.isEmpty()) {
			...
			String[] orderId = orderIdList.toArray(new String[orderIdList.size()]);
			...
			for (OrderType order : orders) {
				...
				double salesTax = order.getShippingDetails().getSalesTax().getSalesTaxAmount() == null ? 0.0 : order.getShippingDetails().getSalesTax()
				...
				System.out.println("Update Shipping and Payment information associated with transactions that belong to OrderID: " + order.getOrderID());
