...
public class SearchApiExample {
	...
	private static void processCommandLine(CommandLine line, Options options) {
		if (line.hasOption(HELP_OPTION)) {
		...
		else if (line.hasOption(CONSUMER_KEY_OPTION) && line.hasOption(CONSUMER_SECRET_OPTION) && line.hasOption(ACCESS_TOKEN_OPTION)
			...
			final String consumerKeyValue = "pps9akw5t85u";
			final String consumerSecretValue = "rz2R2HpXYQoQasr2";
			final String accessTokenValue = "3357e089-f96e-4162-927b-292e3a48eac1";
			final String tokenSecretValue = "1b787100-4087-4b0e-a6a2-d79f87e7de22";
			...
			if (!searchParameters.isEmpty()) {
				...
				System.out.println("Searching for users with factes.");
				List<Parameter<FacetType, String>> facets = new ArrayList<Parameter<FacetType, String>>();
				facets.add(new Parameter<FacetType, String>(FacetType.INDUSTRY,IndustryCodes.Accounting));
...
//				facets.add(new Parameter<FacetType, String>(FacetType.LANGUAGE, LanguageCodes.ENGLISH));
				PeopleSearch people = client.searchPeople(searchParameters,
						EnumSet.of(ProfileField.FIRST_NAME, ProfileField.LAST_NAME, ProfileField.ID, ProfileField.HEADLINE,ProfileField.PUBLIC_PROFILE_URL),
						EnumSet.of(FacetField.NAME, FacetField.CODE),facets);
	...
	private static void printHelp(Options options) {
		...
		String header = MessageFormat.format("\nThe -{0}, -{1}, -{2} and -{3} options are required. All others are optional.", CONSUMER_KEY_OPTION,
				CONSUMER_SECRET_OPTION, ACCESS_TOKEN_OPTION, ACCESS_TOKEN_SECRET_OPTION);
	...
	private static Map<SearchParameter, String> getSearchParameters(CommandLine line) {
		...
		searchParameters.put(SearchParameter.COUNTRY_CODE, "cn");
//
